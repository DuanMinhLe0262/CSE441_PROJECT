        ...
        if (avatarUri != null) {
            ...
            avatarRef.putFile(avatarUri)
                    .addOnSuccessListener(taskSnapshot -> avatarRef.getDownloadUrl().addOnSuccessListener(uri -> {
                        ...
                        saveUserData(new User(mAuth.getCurrentUser().getUid(), name, gender, birthDate, phone, email, avatarUrl, "admin"));
                    }))
                    .addOnFailureListener(e -> Toast.makeText(ChangeProfileActivity.this, "Lỗi upload ảnh đại diện", Toast.LENGTH_SHORT).show());
        } else {

            saveUserData(new User(mAuth.getCurrentUser().getUid(), name, gender, birthDate, phone, email, existingAvatarUrl,"admin"));
        }
    }
